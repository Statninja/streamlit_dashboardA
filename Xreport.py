# app.py
# FULL DISASTER INTELLIGENCE DASHBOARD
# Auto-installs ALL required packages on Streamlit Cloud
...
# =============================================
# 1. AUTO-INSTALL MISSING PACKAGES (ONE-TIME)
# =============================================
...
if "installed_packages" not in st.session_state:
    st.session_state.installed_packages = False

if not st.session_state.installed_packages:
    with st.spinner("Installing required packages... (this takes ~60 seconds)"):
        for pkg in REQUIRED_PACKAGES:
            try:
                __import__(pkg.split(">")[0].split("=")[0])
            except ImportError:
                install_package(pkg)
    st.session_state.installed_packages = True
    st.success("All packages installed! Refreshing...")
    st.experimental_rerun()

# =============================================
# 2. NOW IMPORT EVERYTHING SAFELY
# =============================================
import requests
import pandas as pd
import numpy as np
...
if 'auth' not in st.session_state:
    st.session_state.update({'auth': False, 'saved': [], 'lat': 37.7749, 'lon': -122.4194})
...
def geocode(q): 
    try: loc = geo.geocode(q, timeout=10); return loc and {"lat":loc.latitude,"lon":loc.longitude,"addr":loc.address}
    except: return None
def rev(lat, lon):
...
def fetch_eq(lat, lon, r, s, e, m):
    p = {"format":"geojson","starttime":s,"endtime":e,"latitude":lat,"longitude":lon,
         "maxradiuskm":r,"minmagnitude":m,"limit":20000,"eventtype":"earthquake"}
    try: return requests.get("https://earthquake.usgs.gov/fdsnws/event/1/query", params=p, timeout=30).json()
    except: return None
...
def process_eq(data, ulat, ulon):
    ...
    for f in data["features"]:
        ...
        rows.append({"time": datetime.fromtimestamp(p["time"]/1000), "mag": p["mag"],
                     "depth": c[2], "dist": round(dist,1), "place": p["place"]})
    return pd.DataFrame(rows).sort_values("time", ascending=False)

def aftershock(mag, days):
    K = 10 ** (mag - 4.5); rate = K / (0.05 + days) ** 1.1
...
def fetch_weather(lat, lon):
    ...
    try: return requests.get("https://api.open-meteo.com/v1/forecast", params=p, timeout=30).json()
    except: return None

def fetch_alerts(lat, lon):
    try:
        r = requests.get(f"https://api.weather.gov/alerts/active?point={lat},{lon}",
                         headers={"User-Agent": "PanditaData/1.0", "Accept": "application/geo+json"}, timeout=30).json()
...
def fetch_space():
    try:
        kp = requests.get("https://services.swpc.noaa.gov/json/planetary_k_index_1m.json", timeout=10).json()[-1]["kp_index"]
        return {"kp": kp}
    except: return {"kp": 0}
...
def fetch_hurricanes():
    try:
        ...
        return [{"name": s["name"], "cat": s.get("classification","TD")} for s in r.get("activeStorms",[])]
    except: return []
...
def make_pdf(lat, lon, eq, w, alerts, space):
    ...
    w, h = letter; y = h - 50
    c.setFont("Helvetica-Bold", 16); c.drawString(50, y, "Disaster Report"); y -= 40
    c.setFont("Helvetica", 12); c.drawString(50, y, f"Location: {rev(lat, lon)}"); y -= 40
    if not eq.empty:
        c.drawString(50, y, "Recent Earthquakes:"); y -= 20
...
if not df.empty:
    ...
    c1.metric("Events", len(df)); c2.metric("Max Mag", f"{df['mag'].max():.1f}"); c3.metric("Closest", f"{df['dist'].min():.0f} km")
    main = df.iloc[0]; days_since = (datetime.now() - main["time"]).days + 1
    ash = aftershock(main["mag"], days_since)
    st.info(f"Aftershock Risk: {ash['rec']} | {ash['prob']}% in 7 days")

    fig = px.scatter_mapbox(df, lat="dist", lon="dist", color="mag", size="mag", hover_name="place",
...
if w:
    ...
    c1, c2 = st.columns(2)
    c1.metric("Temp", f"{cw['temperature']:.1f}°C"); c2.metric("Wind", f"{cw['windspeed']:.0f} km/h")
...
if alerts:
    ...
    for a in alerts:
        cls = "critical-alert" if a["severity"] == "Extreme" else "high-alert" if a["severity"] == "Severe" else "moderate-alert"
        st.markdown(f"<div class='{cls}'>{a['event']}: {a['headline']}</div>", unsafe_allow_html=True)

space = fetch_space()
st.subheader("Space Weather"); st.metric("Kp Index", f"{space['kp']:.1f}")

hur = fetch_hurricanes()
if hur:
    st.subheader("Hurricanes")
    for h in hur: st.metric(h["name"], h["cat"])
...
st.markdown("<div class='footer'>© 2025 PanditaData</div>", unsafe_allow_html=True)
